(function() {
  var Bitstream, Buffer, BufferList, CAFDemuxer, Float32, Float64, FromFloat32, FromFloat64, HTTPSource, HTTPWorker, Queue, Stream, ToFloat32, ToFloat64;
  ...
  HTTPWorker = (function() {
    var BlobBuilder, URL, emit, onmessage;

    BlobBuilder = window.BlobBuilder || window.WebKitBlobBuilder || window.MozBlobBuilder;

    URL = window.URL || window.webkitURL;

    function HTTPWorker(doLoop) {
      var bb, url;
      var _this = this;
      this.events = {};
      bb = new BlobBuilder();
      ...
      this.worker.onmessage = function(e) {
        return _this.emit.apply(_this, e.data);
      };
    }

    onmessage = function(e) {
      if (e.data[0] === 'loop') {
        return doLoop.apply(null, Array.prototype.slice.call(e.data, 1));
      }
    };

    emit = function() {
      return postMessage(Array.prototype.slice.call(arguments));
    };

    HTTPWorker.prototype.run = function() {
      var args;
      args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
      return this.worker.postMessage(['loop'].concat(args));
    };

    HTTPWorker.prototype.on = function(evt, fn) {
      var _base, _ref;
      if ((_ref = (_base = this.events)[evt]) == null) _base[evt] = [];
      return this.events[evt].push(fn);
    };

    HTTPWorker.prototype.off = function(evt, fn) {
      var events, index;
      events = this.events[evt];
      if (!events) return;
      index = events.indexOf(fn);
      if (~index) return events.splice(index, 1);
    };

    HTTPWorker.prototype.once = function(evt, fn) {
      var fun;
      var _this = this;
      return this.on(evt, fun = function() {
        _this.off(evt, fun);
        return fn.apply(null, arguments);
      });
    };

    HTTPWorker.prototype.emit = function() {
      var args, events, evt, fn, _i, _len;
  ...
  HTTPSource = (function() {
    ...
    function HTTPSource(name) {
      ...
      this.inflight = false;
      this.reset();
      this.worker = new HTTPWorker(workerLoop);
    }
    ...
    HTTPSource.prototype.loop = function() {
      ...
      this.inflight = true;
      this.worker.once('load', function(response) {
        var buffer;
      ...
      this.worker.once('error', function() {
        console.log("HTTP Error: ", event);
        return _this.pause();
      });
      this.worker.once('abort', function() {
        console.log("HTTP Aborted: Paused?");
        return _this.inflight = false;
      });
      endPos = Math.min(this.offset + this.chunkSize, this.length);
      this.worker.run(this.url, this.offset, endPos);
      return this;
    };

    workerLoop = function(url, start, end) {
      var xhr;
      var _this = this;
      xhr = new XMLHttpRequest();
      xhr.onload = function(event) {
        return emit('load', new Uint8Array(xhr.response));
      };
