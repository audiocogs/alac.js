(function() {
  ...
  Stream = (function() {
    ...
    Stream.prototype.advance = function(bytes) {
      ...
      this.offset += bytes;
      while (this.localOffset > this.list.first.length) {
    ...
    Stream.prototype.readBuffer = function(length) {
      var buffer, i, offset, result, to, _ref, _ref2, _ref3, _ref4;
      result = Buffer.allocate(length);
      to = result.data;
      offset = 0;
      buffer = this.list.first.data;
      while (!(length - offset < buffer.length - this.localOffset)) {
        for (i = _ref = this.localOffset, _ref2 = buffer.length; i < _ref2; i += 1) {
          to[offset - this.localOffset + i] = buffer[i];
        }
        offset += buffer.length - this.localOffset;
        this.advance(buffer.length - this.localOffset);
        buffer = this.list.first.data;
        this.localOffset = 0;
      }
      for (i = _ref3 = this.localOffset, _ref4 = this.localOffset + length - offset; i < _ref4; i += 1) {
        to[offset - this.localOffset + i] = buffer[i];
      }
      this.localOffset = length - offset;
      return result;
    };
  ...
  CAFDemuxer = (function() {
    ...
    CAFDemuxer.prototype.enqueueBuffer = function(buffer) {
      ...
      while (this.headerCache || this.stream.available(12)) {
        if (!this.headerCache) {
          this.headerCache = {
        ...
        switch (this.headerCache.type) {
          case 'kuki':
            ...
            break;
          case 'data':
            while (this.headerCache) {
              if (!this.list.first) return;
              if (this.stream.localOffset + this.headerCache.size > this.list.first.length) {
                buffer = this.list.shift();
                this.outputs.data.send(buffer.slice(this.stream.localOffset));
                this.headerCache.size -= buffer.length - this.stream.localOffset;
                this.stream.localOffset = 0;
              } else {
                buffer = this.list.first;
                this.outputs.data.send(buffer.slice(this.stream.localOffset, this.headerCache.size));
                this.stream.localOffset += this.headerCache.size;
                this.headerCache = null;
              }
            }
            break;
