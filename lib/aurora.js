(function() {
  ...
  Bitstream = (function() {
    ...
    Bitstream.prototype.readBig = function(bits) {
      var a;
      a = this.stream.peekInt32(0) * 0x0100 + this.stream.peekUInt8(4);
      a = a % Math.pow(2, 40 - this.pos);
      this.advance(bits);
      return a / Math.pow(2, 40 - this.pos - bits);
    };

    Bitstream.prototype.peekBig = function(bits) {
      var a;
      a = this.stream.peekUInt32(0) * 0x0100 + this.stream.peekUInt8(4);
      a = a % Math.pow(2, 40 - this.pos);
      return a / Math.pow(2, 40 - this.pos - bits);
    };
    ...
    Bitstream.prototype.read = function(bits) {
      ...
      this.advance(bits);
      return ((a << this.pos) & 0xFFFFFF) >>> (32 - bits);
    };
      ...
      while ((this.headerCache && this.stream.available(1)) || this.stream.available(13)) {
        ...
        switch (this.headerCache.type) {
          ...
          case 'data':
            ...
            console.log("DATA", buffer.data[0], buffer.data[1], buffer.data[2]);
