(function() {
  var Bitstream, Buffer, BufferList, CAFDemuxer, Float32, Float64, FromFloat32, FromFloat64, HTTPSource, Queue, Stream, TWO_16, TWO_24, TWO_32, TWO_8, ToFloat32, ToFloat64;
  ...
  Bitstream = (function() {
    ...
    Bitstream.prototype.readBig = function(bits) {
      var a;
      a = this.stream.peekUInt8(0) * TWO_32 + this.stream.peekUInt8(1) * TWO_24 + this.stream.peekUInt8(2) * TWO_16 + this.stream.peekUInt8(3) * TWO_8 + this.stream.peekUInt8(4) + (a = a % Math.pow(2, 40 - this.pos));
    ...
    Bitstream.prototype.read = function(bits) {
      var a;
      a = (this.stream.peekUInt8(0) << 16) + (this.stream.peekUInt8(1) << 8) + (this.stream.peekUInt8(2) << 0);
      this.advance(bits);
      return ((a << this.pos) & 0xFFFF) >>> (24 - bits);
    };

    Bitstream.prototype.readSmall = function(bits) {
      var a;
      a = (this.stream.peekUInt8(0) << 8) + (this.stream.peekUInt8(1) << 0);
      this.advance(bits);
      return ((a << this.pos) & 0xFF) >>> (16 - bits);
    };
