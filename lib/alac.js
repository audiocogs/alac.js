(function() {
  ...
  ALACDecoder = (function() {
    ...
    ALACDecoder.prototype.decode = function(input, offset, samples, channels) {
      var agParams, bits1, bytesShifted, chanBits, channelIndex, coefsU, coefsV, data, denShiftU, denShiftV, elementInstanceTag, escapeFlag, extraBits, headerByte, i, maxBits, mixBits, mixRes, modeU, modeV, numU, numV, out16, outSamples, output, params, partialFrame, pbFactorU, pbFactorV, shift, shiftbits, status, tag, unused, unusedHeader, val, _ref;
      if (!(channels > 0)) {
        ...
        switch (tag) {
          ...
          case ID_CPE:
            ...
            if (escapeFlag === 0) {
              ...
              status = Aglib.dyn_decomp(agParams, bits, this.predictor, samples, chanBits, data);
            ...
            switch (this.config.bitDepth) {
              case 16:
                out16 = new Uint16Array(output, samples * channelIndex * this.config.bitDepth / 8);
  ...
  Aglib = (function() {
    ...
    Aglib.dyn_decomp = function(params, input, pc, samples, maxSize) {
      var bitPos, c, data, del, j, k, kb, m, maxPos, mb, multiplier, mz, n, ndecode, out, outPtr, pb, startPos, status, wb, zmode, _ref, _ref2;
      pb = params.pb, kb = params.kb, wb = params.wb, mb = params.mb0;
      data = input.data, bitPos = input.pos, maxPos = input.length;
      startPos = bitPos;
  ...
  Matrixlib = (function() {
    ...
    Matrixlib.unmix16 = function(u, v, out, stride, samples, mixbits, mixres) {
      var i, l, u_a, v_a, _results, _results2;
      ...
      if (mixres === 0) {
        ...
        for (i = 0; 0 <= samples ? i < samples : i > samples; 0 <= samples ? i++ : i--) {
          out[i * stride + 0] = u[i];
          _results.push(out[i * stride + 1] = v[i]);
        }
        return _results;
      } else {
        _results2 = [];
        for (i = 0; i < samples; i += 1) {
          ...
          out[i * stride + 0] = l;
          _results2.push(out[i * stride + 1] = l - v[i]);
        }
        return _results2;
      }
    };
