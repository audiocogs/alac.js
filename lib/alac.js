(function() {
  ...
  ALACDecoder = (function() {
    ...
    ALACDecoder.prototype.decode = function(data, offset, samples, channels) {
      var agParams, bits1, bits2, bytesShifted, chanBits, channelIndex, coefsU, coefsV, denShiftU, denShiftV, elementInstanceTag, escapeFlag, extraBits, headerByte, i, maxBits, mixBits, mixRes, modeU, modeV, numU, numV, out16, outSamples, output, params, partialFrame, pb, pbFactorU, pbFactorV, shift, shiftbits, status, tag, unused, unusedHeader, val, _ref, _ref2, _ref3;
      if (!(channels > 0)) {
        ...
        switch (tag) {
          ...
          case ID_CPE:
            ...
            if (escapeFlag === 0) {
              ...
              _ref = Aglib.dyn_decomp(agParams, data, this.predictor, samples, chanBits), status = _ref[0], bits1 = _ref[1];
              if (status !== ALAC.errors.noError) {
                console.log("Mom also said there should be no error");
                return status;
              }
              if (modeU === 0) {
                Dplib.unpc_block(this.predictor, this.predictor, samples, coefsU, numU, chanBits, denShiftU);
              } else {
                Dplib.unpc_block(this.predictor, this.predictor, samples, null, 31, chanBits, 0);
                Dplib.unpc_block(this.predictor, this.mixBufferU, samples, coefsU, numU, chanBits, denShiftU);
              }
              agParams = Aglib.ag_params(this.config.mb, (pb * pbFactorV) / 4, this.config.kb, samples, samples, this.config.maxRun);
              _ref2 = Aglib.dyn_decomp(agParams, data, this.predictor, samples, chanBits), status = _ref2[0], bits2 = _ref2[1];
              if (status !== ALAC.errors.noError) {
                console.log("Mom also said there should be no error");
                return status;
              }
              if (modeU === 0) {
                Dplib.unpc_block(this.predictor, this.predictor, samples, coefsV, numV, chanBits, denShiftV);
              } else {
                Dplib.unpc_block(this.predictor, this.predictor, samples, null, 31, chanBits, 0);
                Dplib.unpc_block(this.predictor, this.mixBufferU, samples, coefsV, numV, chanBits, denShiftV);
              }
            } else {
            ...
            if (bytesShifted !== 0) {
              shift = bytesShifted * 8;
              for (i = 0, _ref3 = samples * 2; i < _ref3; i += 2) {
  ...
  Aglib = (function() {
    var BITOFF, KB0, MAX_DATATYPE_BITS_16, MAX_PREFIX_16, MAX_PREFIX_32, MAX_RUN_DEFAULT, MB0, MDENSHIFT, MMULSHIFT, MOFF, N_MAX_MEAN_CLAMP, N_MEAN_CLAMP_VAL, PB0, QB, QBSHIFT, dyn_get_16, dyn_get_32, get_next, get_stream_bits, lead, lg3a, read;
    ...
    read = function(buf, offset) {
      return (buf[offset++] << 24) | (buf[offset++] << 16) | (buf[offset++] << 8) | buf[offset++];
    };
    ...
    get_stream_bits = function(data, offset, bits) {
      var byteoffset, input, load1, load2, load2shift, result;
      input = data.data;
      byteoffset = offset / 8;
    ...
    dyn_get_32 = function(data, pos, m, k, maxbits) {
      var input, result, stream, v;
      input = data.data;
      stream = read(input, data.offset + pos >> 3);
      stream <<= pos & 7;
      result = lead(~stream);
    ...
    Aglib.dyn_decomp = function(params, data, pc, samples, maxSize) {
      var bitPos, c, del, j, k, kb, m, mb, multiplier, mz, n, ndecode, out, outPtr, pb, startPos, status, wb, zmode, _ref, _ref2;
      ...
      while (c < samples) {
        if (!(bitPos < maxPos)) return ALAC.errors.paramError;
        m = mb >> QBSHIFT;
        k = lg3a(m);
        k = Math.min(k, kb);
        m = (1 << k) - 1;
        _ref = dyn_get_32(data, bitPos, m, k, maxSize), n = _ref[0], bitPos = _ref[1];
        ndecode = n + zmode;
  ...
  Data = (function() {
    ...
    Data.prototype.struct = function(properties) {
      var key, out, val;
      out = {};
      for (key in properties) {
        val = properties[key];
        if (val.slice(0, 7) === 'string[') {
          out[key] = this.readString(+val.slice(7, -1));
        } else {
  ...
  Dplib = (function() {
    ...
    Dplib.unpc_block = function(pc1, out, num, coefs, active, chanbits, denshift) {
      var chanshift, coefs_a, dd, del, del0, denhalf, i, j, lim, out_a, pc1_a, prev, sg, sum1, top, _ref;
