(function() {
  ...
  ALACDecoder = (function() {
    function ALACDecoder(magicCookie) {
      var cookie;
      cookie = new Data(magicCookie);
      cookie.pos = 4;
      if (cookie.readString(4) === 'frma') {
        cookie.pos += 12;
      }
      if (cookie.readString(3) === 'alac') {
        cookie.pos += 12;
      }
      this.config = {
      ...
      this.mixBufferU = new Uint32Array(this.config.frameLength * 4);
      this.mixBufferV = new Uint32Array(this.config.frameLength * 4);
      this.predictor = new Uint32Array(this.config.frameLength * 4);
    }
